...
router.get('/', async function (req, res) {
  try {
    ...
    const segments = url_params.ual.split(':');
      ...
      if (args.length !== 3) {
        console.log(`getStates request with invalid ual from ${url_params.api_key}`)
        resp_object = {
    ...
    if (!url_params.network || (url_params.network !== 'otp::testnet' && url_params.network !== 'otp::mainnet')) {
      console.log(`getStates request with invalid network from ${url_params.api_key}`)
      resp_object = {
        result:
    ...
    user = await getOTHUBData(query, params)
      ...
      await othubdb_connection.query(
        query,
        ['COMPLETE',user[0].admin_key, url_params.api_key, type, url_params.network, url_params.app_group, url_params.txn_description, null, url_params.ual, null, null, null, null, 0, null],
