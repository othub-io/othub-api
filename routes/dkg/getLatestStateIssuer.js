...
router.get("/", async function (req, res) {
  try {
    ...
    if (!url_params.ual || url_params.ual === "") {
      console.log(
        `getLatestStateIssuer request with no ual from ${url_params.api_key}`
      );
      resp_object = {
        status: "400",
    ...
    if (args.length !== 3) {
      console.log(
        `getLatestStateIssuer request with invalid ual from ${url_params.api_key}`
      );
      resp_object = {
        status: "400",
    ...
    if (
      ...
      console.log(
        `getLatestStateIssuer request with invalid network from ${url_params.api_key}`
      );
      resp_object = {
        status: "400",
        result:
